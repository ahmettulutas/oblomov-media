...
export async function POST (request: NextRequest, response: NextResponse<any>) {
  ...
  if(!service_id || !template_id || !user_id) return NextResponse.json({ message: "Api keys for email service have not been found." }, { status: 400 });
  
  try {
    await emailjs.sendForm(service_id, template_id, { ...body }, user_id);
    return NextResponse.json({ message: "Form başarılı bir şekilde gönderildi." }, { status: 201 });
  } catch(err) {
    return NextResponse.json({ message: "Form gönderilirken bir hata meydana geldi." }, { status: 500 });
  }
}
